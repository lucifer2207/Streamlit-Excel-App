...
def process_excel(file):
    ...
    activation_overdue_sites = df.loc[df['ACTIVATION COMPLETE'].str.contains('Overdue', na=False) & df['ACTIVATION COMPLETE'].str.extract(r'(\d+)').astype(float).fillna(0).ge(30).any(axis=1), 'SITE NUMBER'].tolist()
    ...
    planned_submission_after_approval_sites = df.loc[df['FIRST SUBMISSION PLANNED'] > df['ALL APPROVALS PLANNED'], 'SITE NUMBER'].dropna().tolist()
    ...
    actual_submission_after_approval_sites = df.loc[df['FIRST SUBMISSION COMPLETE'] > df['ALL APPROVALS COMPLETE'], 'SITE NUMBER'].dropna().tolist()
    ...
    selected_status_sites = df.loc[(df['SITE STATUS'] == 'Selected') & (df['SITE STATUS EFFECTIVE DATE'] <= today - timedelta(days=365)), 'SITE NUMBER'].dropna().tolist()
    
    # 6. Site has been in a SIV Ready Status for over 90 days
    siv_ready_sites = df.loc[(df['SITE STATUS'] == 'SIV Ready') & (df['SITE STATUS EFFECTIVE DATE'] <= today - timedelta(days=90)), 'SITE NUMBER'].dropna().tolist()
...
st.title("Excel Data Analysis Application")
